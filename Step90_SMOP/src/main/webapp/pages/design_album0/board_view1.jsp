...
<%
	...
	try {
		...
		// 이전글
		sql = "select max(seq) seq, subject from album_board1 where seq < ?";
		pstmt = conn.prepareStatement( sql );
		pstmt.setString( 1, seq );
		...
		// 다음글
		sql = "select min(seq) seq, subject from album_board1 where seq > ?";
		pstmt = conn.prepareStatement( sql );
		pstmt.setString( 1, seq );
...
<%
	if( pseq == null ) {
		out.println( "<span class='b'>이전글 | </span><a>이전글은 없습니다.</a>" );				
	} else {
		out.println( "<span class='b'>이전글 | </span><a href='board_view1.jsp?cpage=" + cpage + "&seq=" + pseq + "'>" + psubject + "</a>" );		
	}
%>
		</div>
...
<%
	if( nseq == null ) {
		out.println( "<span class='b'>다음글 | </span><a>다음글은 없습니다.</a>" );				
	} else {
		out.println( "<span class='b'>다음글 | </span><a href='board_view1.jsp?cpage=" + cpage + "&seq=" + nseq + "'>" + nsubject + "</a>" );		
	}
%>
		</div>
		<!-- //이전글 / 다음글 -->
